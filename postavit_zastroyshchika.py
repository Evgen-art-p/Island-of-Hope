...
# ZASTROYSHCHIK_V1
"""
ЗАСТРОЙЩИК ОСТРОВА — страница, на которой остров строится.

    python postavit_zastroyshchika.py           посмотреть
    python postavit_zastroyshchika.py --sdelat  поставить

Запускать ИЗ КОРНЯ ОСТРОВА.

ЗАЧЕМ

    Жителей на острове не рожаем — их берём с материка. А локации
    рожать надо: остров будет строиться, и Страницы Жизни здесь нет.
    Без застройщика новое место завести нечем.

ЧТО СТАВИТ

    · `ui_zastroyshchik.py` — страница `/zastroyshchik`;
    · маршрут в `ostrov_main.py`.

КАК УСТРОЕНА СТРАНИЦА

    Слева — локации острова, с поиском. Справа — паспорт: тот же, что
    в городе, все поля до единого. На виду четыре, которыми место
    опознаётся: имя, район, чем оно и картинка. Остальное разложено по
    четырём свёрткам — как выглядит, чем дышит, как устроено, где на
    карте. Свёрнуто, но никуда не делось.

    Служебное застройщик заполняет сам: номер, класс объекта, дату,
    подпись, отпечаток. Руками это писать незачем.

НОМЕРА

    Материк нумерует свои локации с нуля: 0014_EXCHANGE и так далее.
    Остров нумерует СВОИ с тысячи: 1001, 1002... Так при обмене через
    Маяк ни один островной номер не наложится на материковый.
"""
import argparse
import ast
import py_compile
...
STRANICA = KOREN / "ui_zastroyshchik.py"
BAK = ".bak_zastroyshchik"


UI_ZASTROYSHCHIK = r'''# -*- coding: utf-8 -*-
# ZASTROYSHCHIK_V1
"""
...
STAROE_MAIN = '''@ui.page("/rabota")
def _rabota():
    page_rabota()
'''
NOVOE_MAIN = '''@ui.page("/rabota")
def _rabota():
    page_rabota()


# ZASTROYSHCHIK_V1 — здесь остров обзаводится местами
from ui_zastroyshchik import page_zastroyshchik   # noqa: E402


@ui.page("/zastroyshchik")
def _zastroyshchik():
    page_zastroyshchik()
'''
...
def main() -> int:
    ...
    print("=" * 58)
    print("ЗАСТРОЙЩИК ОСТРОВА" + ("" if a.sdelat else "   [СУХОЙ ПРОГОН]"))
    print("=" * 58)

    if not MAIN.exists():
        print("x не вижу ostrov_main.py — запускай из корня ОСТРОВА")
        return 1
    if not proverit_python(UI_ZASTROYSHCHIK, "ui_zastroyshchik.py"):
        return 1

    tekst = MAIN.read_text(encoding="utf-8")
    uzhe = "page_zastroyshchik" in tekst
    if uzhe:
        print("  ostrov_main.py: маршрут уже есть")
    elif tekst.count(STAROE_MAIN) != 1:
        print("  x ostrov_main.py: не нашёл, куда встать — не трогаю")
        return 1

    print(f"  ui_zastroyshchik.py: "
          f"{'обновится' if STRANICA.exists() else 'ляжет'}")

    if not a.sdelat:
        print("\nЭто был показ. Ставить: "
              "python postavit_zastroyshchika.py --sdelat")
        return 0

    STRANICA.write_text(UI_ZASTROYSHCHIK, encoding="utf-8")
    if not uzhe:
